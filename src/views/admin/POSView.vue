...
const fetchInitialData = async () => {
  try {
    ...
    customers.value = custRes.data.map(c => ({
      ...
      name: c.hoTen,
      phone: c.soDienThoai,
      type: c.vaiTro?.tenVaiTro || 'Khách hàng'
    }));
...
const updateQuantity = async (detailId, quantity) => {
  ...
  if (!detailId) {
    alert("LỖI FRONTEND: detailId bị undefined! Hãy kiểm tra lại biến item.idGhct trong HTML xem có đúng không. Có thể nó là item.id?");
    return;
  }
  ...
  try {
    ...
    alert("Lỗi Backend: " + (error.response?.data?.message || error.message) + ". Hãy mở Spring Boot lên xem log lỗi đỏ!");
  }
};
...
const removeInvoice = async (index) => {
  ...
  if (confirm("Bạn có chắc muốn đóng hóa đơn này?")) {
    try {
      if (inv.id) {
        ...
        if (response.status === 200) {
          ...
          console.log("Xóa thành công khỏi Backend và UI");
        }
      } else {
      ...
      alert("Không thể xóa hóa đơn này khỏi hệ thống. Vui lòng thử lại!");
    }
  }
};
...
const bankInfo = {
  bankId: 'MB',
  accountNo: '773456999999',
  accountName: 'HOANG DINH NGHIA'
};
            ...
            <div class="voucher-section mb-4">
              ...
              <div class="dropdown">
                ...
                >
                  ...
                  <span v-else class="text-danger fw-bold small"><i class="fas fa-ticket-alt me-2"></i>{{ activeInvoice.voucher.maCode }}</span>
                ...
                <ul class="dropdown-menu w-100 shadow-lg border-0 rounded-3 mt-1">
                  ...
                  <li v-if="activeInvoice.voucher">
                    ...
                    <a class="dropdown-item text-center text-muted small py-2" href="#" @click.prevent="selectVoucher({maCode: null})">Hủy áp dụng</a>
                  </li>
                </ul>
              </div>
            </div>
